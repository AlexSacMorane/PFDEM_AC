...
def Sort_Files(np_proc,n_t_PF_moose,i_PF,L_g):
     #sort files generated by MOOSE to different directories

     #master simulation
     os.rename('PF_'+str(i_PF)+'_out.e','Output/PF_'+str(i_PF)+'_out.e')
     os.rename('PF_'+str(i_PF)+'.i','Input/PF_'+str(i_PF)+'.i')
     os.mkdir('Output/PF_'+str(i_PF))
     j = 0
     j_str = index_to_str(j)
     folderpath = Path('PF_'+str(i_PF)+'_other_'+j_str+'.pvtu')
     while folderpath.exists():
         for i_proc in range(np_proc):
            os.rename('PF_'+str(i_PF)+'_other_'+j_str+'_'+str(i_proc)+'.vtu','Output/PF_'+str(i_PF)+'/PF_'+str(i_PF)+'_other_'+j_str+'_'+str(i_proc)+'.vtu')
         os.rename('PF_'+str(i_PF)+'_other_'+j_str+'.pvtu','Output/PF_'+str(i_PF)+'/PF_'+str(i_PF)+'_other_'+j_str+'.pvtu')
         j = j + 1
         j_str = index_to_str(j)
         folderpath = Path('PF_'+str(i_PF)+'_other_'+j_str+'.pvtu')
